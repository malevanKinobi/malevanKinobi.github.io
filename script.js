...
//     scene.add(mesh);
//
//     // Добавляем кубик в группу башни
//     towerGroup.add(mesh);
//
...
// function init() {
//     autopilot = false; // Включаем автопилот для демонстрационного режима
...
//
//     // Создаем группу для башни перед добавлением слоев
...
//     stopRotation();
//
//     // Сбрасываем башню в исходное положение
//     resetTowerGroup();
//
...
//     }
//
//     // Сбрасываем камеру на начальную ортографическую
//     camera = orthoCamera;
//     if (camera) {
...
//
//     if (stack.length > 0) {
//         // Получаем текущий верхний блок после удаления
//         const topLayer = stack[stack.length - 1];
//
...
//         );
//
//         // Убираем всё из физического мира и из сцены для этого блока
...
//     is_started = false;
//
//     // Переключаем камеру на перспективную
...
//
//     // Ортографическая камера (по умолчанию)
//     const width = 7;
//     const height = width / aspect;
//     orthoCamera = new THREE.OrthographicCamera(width / -2, width / 2, height / 2, height / -2, 0, 100);
//     orthoCamera.position.set(4, 4, 4);
//     orthoCamera.lookAt(0, 0, 0);
//
//     // Перспективная камера (для проигрыша)
...
// }
//
// // Функция для отображения всей пирамиды
...
//
//     // Угол камеры
//     const angle = 30; // Угол в градусах
//     const radianAngle = THREE.MathUtils.degToRad(angle); // Переводим угол в радианы
//
//     // Устанавливаем позицию камеры
//     let distance = totalHeight; // Множитель для отдаления камеры, чтобы захватить всю башню
//
//     if (totalHeight <= 2) { distance *= 12}
//     else if (totalHeight <= 10) { distance *= 5}
//     else { distance *= 1.1 }
//
//     const cameraY = totalHeight + distance * Math.sin(radianAngle); // Позиция по оси Y
//     const cameraZ = distance * Math.cos(radianAngle); // Позиция по оси Z
//
//
//     // Устанавливаем камеру на рассчитанные позиции
//     perspectiveCamera.position.set(0, cameraY, cameraZ);
//
//     // Направляем камеру на центр башни
...
//     // Останавливаем анимацию вращения, если она запущена
//     if (animationFrameId) {
//         cancelAnimationFrame(animationFrameId);
//         animationFrameId = null; // Сбрасываем идентификатор, чтобы он не использовался повторно
//     }
// }
//
//
...
//         const previousLayer = stack[stack.length - 2];
//
//         const maxPosition = 10;  // максимальная позиция по оси X или Z
//         const minPosition = -10; // минимальная позиция по оси X или Z
//
...
//         if (boxShouldMove) {
//             // проверяем, достиг ли блок границ экрана
//             if (topLayer.threejs.position[topLayer.direction] > maxPosition ||
//                 topLayer.threejs.position[topLayer.direction] < minPosition) {
//                 // меняем направление
//                 speed *= -1;
//             }
...
//     lastTime = time;
//
//     TWEEN.update(); // обязательно обновляем TWEEN-анимации
// }
//
...
//
//     const totalHeight = boxHeight * stack.length;
//     const cameraDistance = totalHeight * aspect * 1.2;
//
//     camera.position.z = cameraDistance; // Отодвигаем камеру назад для захвата всей пирамиды
//
//     renderer.setSize(window.innerWidth, window.innerHeight);
//     renderer.setPixelRatio(window.devicePixelRatio);
//     renderer.render(scene, camera);
